...
class TestQueryRange:
    ...
    def test_query_all_decisions(self, temp_repo):
    ...
    def test_query_by_x_range(self, temp_repo):
        ...
        results = manager.query_range(x_range=(2, 5))

        assert len(results) == 3  # x=2, 3, 5
        x_values = [d.coordinate.x for d in results]
        assert sorted(x_values) == [2, 3, 5]

    def test_query_by_z_layer(self, temp_repo):
    ...
    def test_query_combined_ranges(self, temp_repo):
        ...
        results = manager.query_range(x_range=(1, 2), y_range=(2, 3), z_range=(1, 1))

        # Should have: x∈{1,2} × y∈{2,3} × z∈{1} = 4 combinations
        assert len(results) == 4

        for d in results:
            assert 1 <= d.coordinate.x <= 2
            assert 2 <= d.coordinate.y <= 3
            assert d.coordinate.z == 1

    def test_query_empty_range(self, temp_repo):
        ...
        results = manager.query_range(x_range=(10, 10))

        assert len(results) == 0

    def test_query_invalid_range(self, temp_repo):
        """Test that invalid ranges (min > max) are rejected."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        with pytest.raises(QueryError, match="min.*max"):
            manager.query_range(x_range=(10, 1))  # min > max

    def test_query_results_sorted(self, temp_repo):
        ...
        assert results[0].coordinate.x == 1  # (1, 5, 1)
        assert results[1].coordinate.x == 2  # (2, 3, 2)
        assert results[2].coordinate.x == 3  # (3, 2, 1)


class TestSearchContent:
    """Test search_content functionality."""

    def test_search_single_term(self, temp_repo):
        """Test searching for a single keyword."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        # Store decisions with different content
        manager.store(VectorCoordinate(x=1, y=2, z=1), "Use PostgreSQL database")
        manager.store(VectorCoordinate(x=2, y=2, z=1), "Use REST API")
        manager.store(VectorCoordinate(x=3, y=2, z=1), "Use MongoDB database")
    ...
    def test_search_multiple_terms_any(self, temp_repo):
        """Test searching for multiple terms (match any)."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        manager.store(VectorCoordinate(x=1, y=2, z=1), "Use PostgreSQL")
        manager.store(VectorCoordinate(x=2, y=2, z=1), "Use Redis cache")
        manager.store(VectorCoordinate(x=3, y=2, z=1), "Use MongoDB")
        manager.store(VectorCoordinate(x=4, y=2, z=1), "Use JWT tokens")
    ...
    def test_search_multiple_terms_all(self, temp_repo):
        """Test searching for multiple terms (match all)."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        manager.store(VectorCoordinate(x=1, y=2, z=1), "Use PostgreSQL database for authentication")
        manager.store(VectorCoordinate(x=2, y=2, z=1), "Use Redis for caching")
        manager.store(VectorCoordinate(x=3, y=2, z=1), "Use authentication middleware")
    ...
    def test_search_no_matches(self, temp_repo):
    ...
    def test_search_partial_words(self, temp_repo):
...
class TestPartialOrder:
    ...
    def test_less_than_different_x(self, temp_repo):
        """Test partial ordering with different x values."""
        from vector_memory.query import PartialOrder

        # x1 < x2, so (x1, y1) < (x2, y2) regardless of y values
        assert PartialOrder.less_than((1, 2), (3, 4)) is True
        assert PartialOrder.less_than((3, 4), (1, 2)) is False

    def test_less_than_same_x_different_y(self, temp_repo):
        """Test partial ordering with same x, different y."""
        from vector_memory.query import PartialOrder

        # x values equal, so depends on y
        assert PartialOrder.less_than((5, 2), (5, 3)) is True  # y2 < y3
        assert PartialOrder.less_than((5, 3), (5, 2)) is False  # y3 > y2

    def test_less_than_equal_coordinates(self, temp_repo):
        """Test partial ordering with equal coordinates."""
        from vector_memory.query import PartialOrder

        # Equal coordinates are not less than each other
        assert PartialOrder.less_than((5, 3), (5, 3)) is False

    def test_comparable_comparable_pairs(self, temp_repo):
        """Test comparable() with comparable coordinate pairs."""
        from vector_memory.query import PartialOrder

        # Coordinates where one is less than the other
        assert PartialOrder.comparable((1, 2), (3, 4)) is True
        assert PartialOrder.comparable((5, 2), (5, 3)) is True
        assert PartialOrder.comparable((5, 3), (5, 2)) is True

    def test_comparable_incomparable_pairs(self, temp_repo):
        """Test comparable() with incomparable coordinate pairs."""
        from vector_memory.query import PartialOrder

        # Coordinates that are neither < nor > each other
        # (3, 4) and (5, 2): x1 < x2 but we can't compare y values directly
        # Actually (3,4) < (5,2) because x1 < x2, so they ARE comparable
        assert PartialOrder.comparable((3, 4), (5, 2)) is True

        # For truly incomparable, we'd need non-integer coordinates
        # With our definition, all integer pairs are comparable

    def test_find_before_filters_correctly(self, temp_repo):
        """Test find_before() filters coordinates correctly."""
        from vector_memory.query import PartialOrder
        ...
        # Find all before (7, 4)
        result = PartialOrder.find_before(coords, (7, 4))

        # Should include everything except (7, 4) itself
        expected = [
            (1, 2, 1),
            (3, 2, 1),
            (3, 4, 2),
    ...
    def test_find_before_empty_result(self, temp_repo):
        """Test find_before() with no coordinates before threshold."""
        from vector_memory.query import PartialOrder
        ...
        # Find all before (1, 1) - should be empty
        result = PartialOrder.find_before(coords, (1, 1))
...
class TestPartialOrderQueries:
    ...
    def test_query_partial_order_basic(self, temp_repo):
        ...
        coords = [
            VectorCoordinate(x=1, y=2, z=1),
            VectorCoordinate(x=3, y=2, z=1),
            VectorCoordinate(x=3, y=4, z=1),
            VectorCoordinate(x=5, y=2, z=1),
            VectorCoordinate(x=7, y=2, z=1),
            VectorCoordinate(x=7, y=4, z=1),
        ]
        ...
        results = manager.query_partial_order(x_threshold=7, y_threshold=4)
        ...
        for decision in results:
            ...
            assert x < 7 or (x == 7 and y < 4)

    def test_query_partial_order_with_z_filter(self, temp_repo):
        """Test partial order query with z-layer filter."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        # Store decisions at multiple layers
        for z in [1, 2, 3]:
            manager.store(VectorCoordinate(x=1, y=2, z=z), f"Layer {z}")
            manager.store(VectorCoordinate(x=3, y=2, z=z), f"Layer {z}")

        # Query for architecture layer (z=1) only before (5, 1)
        results = manager.query_partial_order(x_threshold=5, y_threshold=1, z_filter=1)

        # Should get (1,2,1) and (3,2,1) but not other layers
        assert len(results) == 2
        assert all(d.coordinate.z == 1 for d in results)

    def test_query_partial_order_empty(self, temp_repo):
        """Test partial order query with no matches."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        # Store decision at (5, 3)
        manager.store(VectorCoordinate(x=5, y=3, z=1), "Test")

        # Query for decisions before (1, 1) - should be empty
        results = manager.query_partial_order(x_threshold=1, y_threshold=1)

        assert len(results) == 0

    def test_query_partial_order_results_sorted(self, temp_repo):
        ...
        results = manager.query_partial_order(x_threshold=10, y_threshold=1)
