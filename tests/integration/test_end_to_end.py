...
class TestStoreAndRetrieve:
    ...
    def test_store_and_retrieve_decision(self, temp_repo):
    ...
    def test_retrieve_nonexistent_coordinate(self, temp_repo):
        """Test retrieving from empty coordinate returns None."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        coord = VectorCoordinate(x=10, y=3, z=2)
        result = manager.get(coord)

        assert result is None

    def test_exists_check(self, temp_repo):
    ...
    def test_store_multiple_coordinates(self, temp_repo):
    ...
    def test_overwrite_mutable_layer(self, temp_repo):
        """Test that mutable layers can be overwritten."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        coord = VectorCoordinate(x=5, y=3, z=3)  # z=3 is mutable
    ...
    def test_immutable_layer_enforcement(self, temp_repo):
        """Test that z=1 layer cannot be modified."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        coord = VectorCoordinate(x=5, y=2, z=1)  # z=1 is immutable
...
class TestValidation:
    ...
    def test_empty_content_rejected(self, temp_repo):
    ...
    def test_content_too_large_rejected(self, temp_repo):
    ...
    def test_invalid_coordinate_rejected(self, temp_repo):
        """Test that invalid coordinates are rejected."""
        VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        # These should raise CoordinateValidationError during construction
...
class TestManagerInitialization:
    ...
    def test_creates_vector_memory_directory(self, temp_repo):
...
class TestRollbackScenario:
    ...
    def test_rollback_to_safe_state(self, temp_repo):
        """
        ...
        decisions_before_error = manager.query_partial_order(
            x_threshold=error_x, y_threshold=error_y
        )
        ...
        for decision in decisions_before_error:
            ...
            assert x < error_x or (x == error_x and y < error_y)
    ...
    def test_rollback_with_layer_filter(self, temp_repo):
        """
        ...
        for x, y, z, content in decisions:
            manager.store(VectorCoordinate(x=x, y=y, z=z), content)

        # Find only architecture decisions (z=1) before issue 5
        arch_decisions = manager.query_partial_order(x_threshold=5, y_threshold=1, z_filter=1)
        ...
        assert all(d.coordinate.x < 5 for d in arch_decisions)
    ...
    def test_rollback_preserves_immutable_decisions(self, temp_repo):
        """
        ...
        safe_arch_decisions = manager.query_partial_order(
            x_threshold=5, y_threshold=3, z_filter=1  # Architecture layer only
        )
    ...
    def test_incremental_rollback_workflow(self, temp_repo):
        """
        ...
        for x in [1, 2, 3, 5, 7, 10]:
            for y in [2, 3]:
                manager.store(VectorCoordinate(x=x, y=y, z=1), f"Decision at ({x}, {y})")
        ...
        for x_thresh, y_thresh, expected_count in thresholds:
            decisions = manager.query_partial_order(x_threshold=x_thresh, y_threshold=y_thresh)
            assert len(decisions) == expected_count

            # Verify all are before threshold
            for d in decisions:
                x, y = d.coordinate.x, d.coordinate.y
                assert x < x_thresh or (x == x_thresh and y < y_thresh)
