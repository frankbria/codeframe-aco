...
class TestGitSync:
    ...
    def test_sync_creates_commit(self, temp_repo, mock_issue_id):
        ...
        manager.store(VectorCoordinate(x=mock_issue_id(1), y=2, z=1), "Decision 1")
        manager.store(VectorCoordinate(x=mock_issue_id(2), y=2, z=1), "Decision 2")
    ...
    def test_sync_includes_vector_memory_files(self, temp_repo, mock_issue_id):
    ...
    def test_load_from_git_after_restart(self, temp_repo, mock_issue_id):
    ...
    def test_sync_multiple_times(self, temp_repo, mock_issue_id):
        """Test syncing multiple times creates multiple commits."""
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="test-agent")

        # First sync
        manager.store(VectorCoordinate(x=mock_issue_id(1), y=2, z=1), "First")
        manager.sync(message="First commit")

        # Second sync
        manager.store(VectorCoordinate(x=mock_issue_id(2), y=2, z=1), "Second")
        manager.sync(message="Second commit")

        # Third sync
        manager.store(VectorCoordinate(x=mock_issue_id(3), y=2, z=1), "Third")
    ...
    def test_sync_with_custom_message(self, temp_repo, mock_issue_id):
    ...
    def test_sync_without_custom_message(self, temp_repo, mock_issue_id):
    ...
    def test_load_from_git_initializes_index(self, temp_repo, mock_issue_id):
    ...
    def test_recovery_after_crash(self, temp_repo, mock_issue_id):
...
class TestGitIntegration:
    ...
    def test_sync_empty_changes(self, temp_repo, mock_issue_id):
    ...
    def test_load_from_empty_repository(self, temp_repo, mock_issue_id):
    ...
    def test_concurrent_access_same_repo(self, temp_repo, mock_issue_id):
        """Test that multiple managers can work with same repo."""
        # Manager 1 stores some decisions
        manager1 = VectorMemoryManager(repo_path=temp_repo, agent_id="agent-1")
        manager1.store(VectorCoordinate(x=mock_issue_id(1), y=2, z=1), "From agent 1")
        manager1.sync()

        # Manager 2 stores different decisions
        manager2 = VectorMemoryManager(repo_path=temp_repo, agent_id="agent-2")
        manager2.store(VectorCoordinate(x=mock_issue_id(2), y=2, z=1), "From agent 2")
        manager2.sync()

        # Manager 3 should see both decisions
        manager3 = VectorMemoryManager(repo_path=temp_repo, agent_id="agent-3")

        decision1 = manager3.get(VectorCoordinate(x=mock_issue_id(1), y=2, z=1))
        decision2 = manager3.get(VectorCoordinate(x=mock_issue_id(2), y=2, z=1))
