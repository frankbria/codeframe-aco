...
class TestQuickstartExamples:
    ...
    def test_quickstart_example_1_initialize(self, temp_repo, mock_issue_id):
    ...
    def test_quickstart_example_2_store_decision(self, temp_repo, mock_issue_id):
    ...
    def test_quickstart_example_3_retrieve_decision(self, temp_repo, mock_issue_id):
    ...
    def test_quickstart_example_4_sync_to_git(self, temp_repo, mock_issue_id):
    ...
    def test_use_case_1_store_architectural_decision(self, temp_repo, mock_issue_id):
    ...
    def test_use_case_2_query_architecture_decisions(self, temp_repo, mock_issue_id):
    ...
    def test_use_case_3_find_decisions_for_rollback(self, temp_repo, mock_issue_id):
        ...
        decisions_before = manager.query_partial_order(x_threshold=mock_issue_id(15), y_threshold=3)
        ...
        for decision in decisions_before:
            ...
            assert x < mock_issue_id(15) or (x == mock_issue_id(15) and y < 3)

    def test_use_case_4_search_for_specific_topics(self, temp_repo, mock_issue_id):
    ...
    def test_use_case_5_update_implementation_details(self, temp_repo, mock_issue_id):
        """Test: Use Case 5 - Update Implementation Details."""
        # From quickstart.md "Use Case 5"
        manager = VectorMemoryManager(repo_path=temp_repo, agent_id="claude-code-01")

        coord = VectorCoordinate(x=mock_issue_id(10), y=3, z=3)
    ...
    def test_pattern_start_of_new_issue(self, temp_repo, mock_issue_id):
        ...
        arch_coord = VectorCoordinate(x=mock_issue_id(issue_num), y=2, z=1)
        manager.store(arch_coord, "Follow REST API patterns from issue #5")

        # Store test decisions
        test_coord = VectorCoordinate(x=mock_issue_id(issue_num), y=2, z=2)
    ...
    def test_pattern_context_retrieval_for_new_agent(self, temp_repo, mock_issue_id):
        ...
        context = manager.query_range(x_range=(mock_issue_id(1), mock_issue_id(49)), z_range=(1, 1))

        # Build context string
        context_text = "\n".join([f"Issue {d.coordinate.x}: {d.content}" for d in context])

        assert len(context) == 49
        assert f"Issue {mock_issue_id(1)}:" in context_text
        assert f"Issue {mock_issue_id(49)}:" in context_text

    def test_error_handling_immutable_layer_error(self, temp_repo, mock_issue_id):
        ...
        new_coord = VectorCoordinate(x=mock_issue_id(10), y=2, z=1)
    ...
    def test_coordinate_system_layers(self, temp_repo, mock_issue_id):
